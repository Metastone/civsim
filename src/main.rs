...
impl System for HungerSystem {
    fn run(&self, manager: &mut ArchetypeManager) {
        for (arch_index, entity_index, _) in manager.iter_entities(ComponentType::Person) {
            if let Some(person) = manager.get_component::<PersonComponent>(arch_index, entity_index, &ComponentType::Person) {
...
impl System for DeathSystem {
    fn run(&self, manager: &mut ArchetypeManager) {
        ...
        to_remove.iter().for_each(|entity| manager.remove_entity(*entity));
    }
}
...
impl System for MoveToFoodSystem {
    fn run(&self, manager: &mut ArchetypeManager) {
        ...
        for (arch_index, entity_index, entity) in manager.iter_entities_with(&[ComponentType::Behavior, ComponentType::Position]) {
            if let Some(position) = manager.get_component::<PositionComponent>(arch_index, entity_index, &ComponentType::Position) {
        ...
        for (entity, position) in &behavior_positions {
            ...
            for (arch_index, entity_index, food_entity) in manager.iter_entities_with(&[ComponentType::Food, ComponentType::Position]) {
                if let Some(food_position) = manager.get_component::<PositionComponent>(arch_index, entity_index, &ComponentType::Position) {
...
impl System for EatSystem {
    fn run(&self, manager: &mut ArchetypeManager) {
        ...
        for (arch_index, entity_index, entity) in manager.iter_entities(ComponentType::EatingFood) {
            if let Some(eating_food) = manager.get_component::<EatingFoodComponent>(arch_index, entity_index, &ComponentType::EatingFood) {
        ...
        for (arch_index, entity_index, entity) in manager.iter_entities_with(&[ComponentType::EatingFood, ComponentType::Person]) {
            if let Some(person) = manager.get_component::<PersonComponent>(arch_index, entity_index, &ComponentType::Person) {
...
impl World {
    ...
    pub fn draw(&mut self, pixels: &mut [u8], window_width: u32, window_height: u32) {
        ...
        for (arch_index, entity_index, _) in self.archetype_manager.iter_entities(ComponentType::Position) {
            let (color, placeholder_pixel_size) =
            if let Some(_) = self.archetype_manager.get_component::<PersonComponent>(arch_index, entity_index, &ComponentType::Person) {
                (PERSON_COLOR, PERSON_PLACEHOLDER_PIXEL_SIZE)
            } else {
                (FOOD_COLOR, FOOD_PLACEHOLDER_PIXEL_SIZE)
            };

            if let Some(position) = self.archetype_manager.get_component::<PositionComponent>(arch_index, entity_index, &ComponentType::Position) {
